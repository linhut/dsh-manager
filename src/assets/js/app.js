...
async function renderSkillsPage() {
  ...
  try {
    ...
    el.innerHTML = [
      ...
      '  <div class="card-body" style="font-size:12px;line-height:1.8;color:var(--text-muted);">',
      '    <div>1️⃣ <strong>存放</strong>：技能保存在 <code>~/.dsh/skills</code> 或 <code>customSkillDirs</code> 配置的目录（每个技能一个目录 + <code>SKILL.md</code>）。</div>',
      '    <div>2️⃣ <strong>发现</strong>：DSH 会话启动时，本地 provider 扫描技能目录，把 <strong>名称 + 描述</strong> 注入会话目录。</div>',
      '    <div>3️⃣ <strong>调用</strong>：模型遇到与描述匹配的任务时，通过内置 <code>skill({ name })</code> 工具按需加载技能正文执行。</div>',
      '    <div>4️⃣ <strong>控制</strong>：开关「模型可调用 / 用户可调用」控制触发方式；同名技能按 用户 &gt; 自定义 &gt 项目 &gt; 内置 优先级，被覆盖者标记「被覆盖」。</div>',
      '  </div>',
      '</div>',
