...
function apply(ctx, config) {
	...
	if (config.printUrl || handoffBrowser) ctx.inject(["connection"], (connectionCtx) => {
		...
		if (settled === void 0) announceReady();
			...
			settled.then(announceWhenReady, () => {});
			// Grace fallback: the loader tree can stay busy on unrelated plugin entries
			// (failed UI plugins, stalled MCP stdio handshakes). Do not let them block the
			// authentication URL forever; announce as soon as the web runtime is up.
			setTimeout(announceWhenReady, 8000);
		}
	});
}
//#endregion
