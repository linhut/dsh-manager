...
//#region lib/vendor-patch/plugin-quarantine.js (dsh-manager)
/* dsh-manager vendor patch：插件适配器崩溃隔离信号。
 * 在 adapter 契约缺失（如 prepareCall 未实现）或 prepare 阶段抛错时，
 * 不使会话 turn 裸崩为不可读错误，而是抛出带 provider 上下文的 LlmError，
 * 同时把一条隔离事件追加到 ~/.dsh/manager/plugin-quarantine.jsonl，
 * 由 dsh-manager 主进程监听并自动暂停对应插件、向用户提示。
 * 维护入口：dsh-manager/scripts/sync-vendor-patches.mjs
 */
const quarantineRequire = createRequire(import.meta.url);
const quarantineFs = quarantineRequire("node:fs");
const quarantineOs = quarantineRequire("node:os");
const quarantinePath = quarantineRequire("node:path");
function quarantineEventFile() {
	const home = quarantineOs.homedir();
	const dshHome = (typeof process !== "undefined" && process.env && process.env.DSH_HOME) || quarantinePath.join(home, ".dsh");
	return quarantinePath.join(dshHome, "manager", "plugin-quarantine.jsonl");
}
function quarantineAdapterName(adapter, provider) {
	try {
		if (adapter && typeof adapter.providerInfo === "function") {
			const info = adapter.providerInfo(provider);
			if (info && typeof info.name === "string" && info.name.length > 0) return info.name;
		}
	} catch { }
	return provider;
}
function emitQuarantineEvent(ev) {
	try {
		if (!ev || typeof ev.provider !== "string" || ev.provider.length === 0) return;
		const line = JSON.stringify({
			ts: new Date().toISOString(),
			type: "adapter-crash",
			provider: ev.provider,
			model: typeof ev.model === "string" ? ev.model : "",
			stage: typeof ev.stage === "string" ? ev.stage : "prepareCall",
			adapter: quarantineAdapterName(ev.adapter, ev.provider),
			message: typeof ev.message === "string" ? ev.message.slice(0, 800) : "adapter crash",
		}) + "\n";
		try {
			quarantineFs.appendFileSync(quarantineEventFile(), line, "utf8");
		} catch { }
	} catch { }
}
//#endregion
	...
	return class LlmRuntime extends _classSuper {
		...
		*/
		async prepareCall(config, signal) {
			...
			const registrationAdapter = registration.adapter;
			if (!registrationAdapter || typeof registrationAdapter.prepareCall !== "function") {
				const qMessage = `provider "${config.provider}" 的适配器未实现 prepareCall（插件与 dsh-llm API 代差或适配器损坏）`;
				emitQuarantineEvent({ provider: config.provider, model: config.model, stage: "prepareCall.contract", message: qMessage, adapter: registrationAdapter });
				throw new LlmError(`${qMessage}，已上报隔离信号，请暂停该插件或升级适配器`, "ADAPTER_CONTRACT_MISSING");
			}
			let adapterCall;
			try {
				adapterCall = await registrationAdapter.prepareCall(config.provider, config.model, signal);
			} catch (error) {
				const detail = (error && typeof error.message === "string" ? error.message : String(error)).slice(0, 400);
				const qMessage = `provider "${config.provider}" prepareCall 失败: ${detail}`;
				emitQuarantineEvent({ provider: config.provider, model: config.model, stage: "prepareCall.execute", message: qMessage, adapter: registrationAdapter });
				throw new LlmError(qMessage, "ADAPTER_PREPARE_FAILED");
			}
