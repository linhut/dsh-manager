...
/** Upper bound for a single MCP connect (stdio handshake / initialize). A server
* that starts but never completes the handshake must not wedge the loader tree:
* the attempt fails, the transport is closed, and the reconnect policy takes over. */
...
function startConnection(ctx, config, policy) {
	...
	*/
	...
	/** Bound one connect attempt so a handshake that never completes (e.g. a
	* server that starts but never answers `initialize`) cannot wedge the loader.
	* On timeout the transport is closed and the attempt rejects; the caller's
	* catch path treats it like any other failed attempt. */
	function withConnectTimeout(generation, config, label) {
		...
		return new Promise((resolve, reject) => {
			...
			const timer = setTimeout(() => {
				...
				reject(new Error(`${label}: connect timed out after ${CONNECT_TIMEOUT_MS}ms (no MCP handshake)`));
			}, CONNECT_TIMEOUT_MS);
			if (typeof timer.unref === "function") timer.unref();
			generation.connect(transport).then(() => {
				if (settled) return;
				settled = true;
				clearTimeout(timer);
				resolve();
	...
	*/
	async function connectGeneration(startup) {
		...
		try {
			await withConnectTimeout(generation, config, label);
			if (hasClosed()) {
				attemptSettled = true;
